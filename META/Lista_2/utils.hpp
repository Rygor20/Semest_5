...
std::vector<std::size_t> genRandCycle(std::size_t size, std::mt19937& randGen) {
...
double weightFunction(const vector<size_t>& cycle, vector<Node>& points) {
    int_fast32_t cycleWeight{0};

    for(size_t i = 0; i < cycle.size(); i++) {
        cycleWeight += calcDist(points[cycle[i] - 1], points[cycle[(i + 1) % cycle.size()] - 1]);
    }
